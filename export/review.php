...
if(isset($_REQUEST['f'])){
  ...
  $reviewImage = './imagens/' . substr($_REQUEST['f'],0,-9) . '.png';

  $handle = fopen('./processados/'.$_REQUEST['f'],'r');
  $json = fread($handle,filesize ('./processados/'.$_REQUEST['f']));
  fclose($handle);
  $output = json_decode($json,true);

  $original = imagecreatefromjpeg($output['arquivo']);
